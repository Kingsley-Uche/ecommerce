 ...
 *
 * Also reads item.size from the API response (getCart returns the
 * full CartModel including the size column).
 *
 * Load this AFTER inner.js so it overwrites the original function.
 * ---------------------------------------------------------------
 */
async function CheckoutContent(data) {
    ...
    data.forEach(item => {
        ...
        // Build size options, pre-selecting the stored size
        const sizeOptions = sizes.map(s =>
            `<option value="${s}" ${selectedSize === s ? 'selected' : ''}>
                ${s === '' ? '— Size —' : s}
             </option>`
        ).join('');
                ...
                <div class="row align-items-center g-1">
                    ...
                    <div class="col-lg-2 col-4 text-center mb-2 mb-lg-0">
                        ...
                        <span class="current-price">
                            <strong>₦${price.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</strong>
                        </span>
                    </div>
                    ...
                    <div class="col-lg-1 col-2 text-center item-total">
                        ...
                        <strong>₦${itemTotal.toLocaleString('en-NG', { minimumFractionDigits: 2 })}</strong>
                    </div>
